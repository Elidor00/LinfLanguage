...
public class DeletionTest {
    ...
    @Test
    public void CheckSemantics_ShouldFail_WhenDeletingFunctionInsideItsBody() throws BehaviourError {
        List<SemanticError> errors = checkSemantics(
                "{f(int k);\n" +
                        "\n" +
                        "g(int k) { delete f; }\n" +
                        "\n" +
                        "f(int k) { g(9); delete g; }\n" +
                        "\n" +
                        "f(10);}");
        assertEquals(1, errors.size());
        SemanticError err = errors.get(0);
        assertEquals(FunctionNameDeletionError.class, err.getClass());
        assertEquals("f", ((FunctionNameDeletionError) err).getId());
    }

    @Test
    public void CheckSemantics_ShouldFail_WhenUsingFunctionAfterDeleting() throws BehaviourError {
        List<SemanticError> errors = checkSemantics(
                "{\n" +
                        "\n" +
                        "f(int k) { print k; }\n" +
